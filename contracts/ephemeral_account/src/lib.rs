...
impl EphemeralAccountContract {
    ...
    /// sweep destination for this account. Any subsequent call — even via
    /// `reclaim_reserve` — that would attempt a different destination is rejected
    /// with [`Error::SweepDestinationLocked`].
    ///
    /// # Arguments
    ...
    pub fn sweep(env: Env, destination: Address, auth_signature: BytesN<64>) -> Result<(), Error> {
        ...
        // Enforce single-destination lock: if a destination was previously stored,
        // reject any call with a different address.
        if let Some(locked) = storage::get_sweep_destination(&env) {
            if locked != destination {
                return Err(Error::SweepDestinationLocked);
            }
        } else {
            // First sweep call — persist the destination.
    ...
    ///
    /// Expiry is based on ledger sequence number, not wall-clock time.
